 ...
 */
public class DaysSeekBar<T extends Number> extends ImageView {
    /**
     * Default color of a {@link DaysSeekBar}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
    ...
    private static int DEFAULT_TEXT_SIZE_IN_DP = 10;
    ...
    private String postFix = "", postFixS = "";
    private String preFix = "", preFixS = "";

    public DaysSeekBar(Context context) {
        super(context);
        init(context, null);
    }

    public DaysSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public DaysSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.seekbar_fill_color;
        int thumbPressed = R.drawable.seekbar_fill_color;
        int thumbDisabled = R.drawable.seekbar_fill_color;
        ...
        if (attrs == null) {
            ...
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.DaysSeekBar, 0, 0);
            try {
                setRangeValues(
                        extractNumericValueFromAttributes(a, R.styleable.DaysSeekBar_absoluteMinValue, DEFAULT_MINIMUM),
                        extractNumericValueFromAttributes(a, R.styleable.DaysSeekBar_absoluteMaxValue, DEFAULT_MAXIMUM)
                );
                showTextAboveThumbs = a.getBoolean(R.styleable.DaysSeekBar_valuesAboveThumbs, true);
                textAboveThumbsColor = a.getColor(R.styleable.DaysSeekBar_textAboveThumbsColor, Color.rgb(255, 255, 255)); //Text rgb
                singleThumb = a.getBoolean(R.styleable.DaysSeekBar_singleThumb, false);
                showLabels = a.getBoolean(R.styleable.DaysSeekBar_showLabels, true);
                internalPad = a.getDimensionPixelSize(R.styleable.DaysSeekBar_internalPadding, INITIAL_PADDING_IN_DP);
                barHeight = a.getDimensionPixelSize(R.styleable.DaysSeekBar_barHeight, LINE_HEIGHT_IN_DP);
                activeColor = a.getColor(R.styleable.DaysSeekBar_activeColor, Color.rgb(255, 0, 144)); //Active line color
                defaultColor = a.getColor(R.styleable.DaysSeekBar_defaultColor, Color.rgb(161, 161, 161)); //default color
                alwaysActive = a.getBoolean(R.styleable.DaysSeekBar_alwaysActive, true);
                fontName = a.getString(R.styleable.DaysSeekBar_fontNameSeekBar);

                postFix = a.getString(R.styleable.DaysSeekBar_postFixString);
                postFixS = a.getString(R.styleable.DaysSeekBar_postFixStringS);
                preFix = a.getString(R.styleable.DaysSeekBar_preFixString);
                preFixS = a.getString(R.styleable.DaysSeekBar_preFixStringS);

                DEFAULT_TEXT_SIZE_IN_DP = a.getInteger(R.styleable.DaysSeekBar_textSizeSeekBar, 4);

                Drawable normalDrawable = a.getDrawable(R.styleable.DaysSeekBar_thumbNormal);
                if (normalDrawable != null) {
                    thumbImage = BitmapUtil.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.DaysSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.DaysSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = BitmapUtil.drawableToBitmap(pressedDrawable);
                }
                thumbShadow = a.getBoolean(R.styleable.DaysSeekBar_thumbShadow, false);
                thumbShadowColor = a.getColor(R.styleable.DaysSeekBar_thumbShadowColor, defaultShadowColor);
                thumbShadowXOffset = a.getDimensionPixelSize(R.styleable.DaysSeekBar_thumbShadowXOffset, defaultShadowXOffset);
                thumbShadowYOffset = a.getDimensionPixelSize(R.styleable.DaysSeekBar_thumbShadowYOffset, defaultShadowYOffset);
                thumbShadowBlur = a.getDimensionPixelSize(R.styleable.DaysSeekBar_thumbShadowBlur, defaultShadowBlur);

                activateOnDefaultValues = a.getBoolean(R.styleable.DaysSeekBar_activateOnDefaultValues, true);
            } finally {
                a.recycle();
            }
        }
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(width, height + (textSize * 3));
    }
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (fontName != null) {
            paint.setTypeface(Typeface.createFromAsset(getContext().getAssets(), fontName));
        }
        ...
        if (showLabels) {
            ...
            float minMaxHeight = textOffset + thumbHalfHeight + textSize;
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            String maxText;
            if (getSelectedMaxValue().equals(getAbsoluteMinValue()))
                maxText = preFix + valueToString(getSelectedMaxValue()) + postFix;
            else
                maxText = preFix + valueToString(getSelectedMaxValue()) + postFixS;

            String minText;

            if (getSelectedMinValue().equals(getAbsoluteMinValue()))
                minText = preFix + valueToString(getSelectedMinValue()) + postFix;
            else
                minText = preFix + valueToString(getSelectedMinValue()) + postFixS;

            /*if (getSelectedMaxValue().equals(getAbsoluteMaxValue()))
                maxText = prefix + maxText + "+";
            else maxText = prefix + maxText;*/
    ...
    public interface OnRangeSeekBarChangeListener<T extends Number> {
        ...
        void onRangeSeekBarValuesChanged(DaysSeekBar<T> bar, T minValue, T maxValue);
    }
