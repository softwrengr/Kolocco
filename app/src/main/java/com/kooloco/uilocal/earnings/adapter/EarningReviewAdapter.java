...
public class EarningReviewAdapter extends RecyclerView.Adapter<EarningReviewAdapter.ViewHolder> {
    Context context;
    List<Review> reviews;


    public EarningReviewAdapter(Context context, List<Review> reviews) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.row_local_earning_review, parent, false));
    }

    @Override
    public void onBindViewHolder(ViewHolder holder, final int position) {
        Picasso.with(context).load(reviews.get(position).getImageUrl()).resizeDimen(R.dimen.dp_60,R.dimen.dp_60).centerCrop().transform(new CircleTransform()).into(holder.imageViewProfile);
